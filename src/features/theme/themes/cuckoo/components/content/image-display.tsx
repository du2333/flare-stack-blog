...
export function ImageDisplay({
  ...
  return (
    <figure className="not-prose my-8 group relative block cuckoo-onload-animation select-none overflow-hidden rounded-xl">
      {/* Image Container with Cuckoo aesthetics */}
      <div className="relative">
        {/* Dark mode overlay to reduce glow - matching original Cuckoo */}
      ...
      {alt && (
        <figcaption className="mt-4 p-0 flex items-center justify-center gap-2 px-4 text-center">
          <span className="text-sm font-medium cuckoo-text-50 transition-colors duration-500 group-hover:cuckoo-text-75 select-none">
            {alt}
          </span>
        </figcaption>
      )}
