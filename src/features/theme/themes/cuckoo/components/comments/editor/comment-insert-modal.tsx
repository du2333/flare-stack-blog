...
interface CuckooInsertModalProps {
  type: ModalType;
  initialUrl?: string;
  onClose: () => void;
...
const CuckooInsertModalInternal: React.FC<CuckooInsertModalProps> = ({
  type,
  initialUrl = "",
  onClose,
  onSubmit,
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={`
          relative w-full max-w-md cuckoo-card-base p-0
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold cuckoo-text-90">
            {activeType === "LINK"
          ...
          <button
            onClick={onClose}
            className="p-1 -mr-1 cuckoo-text-30 hover:cuckoo-text-75 transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-4 flex flex-col gap-4">
          <div className="space-y-2">
            <label className="text-sm font-medium cuckoo-text-75 ml-1">
              {activeType === "IMAGE"
            ...
            <input
              ...
              placeholder="https://..."
              className="w-full px-4 py-2.5 rounded-xl border border-(--cuckoo-input-border) bg-(--cuckoo-input-bg) focus:outline-none focus:border-(--cuckoo-primary)/50 transition-all cuckoo-text-90 text-sm"
            />
          </div>

          <p className="text-xs cuckoo-text-30 px-1">
            {activeType === "LINK"
        ...
        <div className="px-6 pb-6 flex justify-end gap-3 mt-2">
          <button
            onClick={onClose}
            type="button"
            className="h-10 px-4 text-sm cuckoo-text-50 hover:cuckoo-text-75 transition-colors font-medium"
          >
          ...
          <button
            ...
            className="cuckoo-btn-primary h-10 px-6 text-sm rounded-xl font-bold disabled:opacity-40 disabled:cursor-not-allowed"
          >
...
export const CuckooInsertModal: React.FC<CuckooInsertModalProps> = (props) => {
  return (
    <ClientOnly>
      <CuckooInsertModalInternal {...props} />
    </ClientOnly>
  );
};
