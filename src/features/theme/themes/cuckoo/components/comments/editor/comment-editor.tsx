...
import CuckooCommentEditorToolbar from "./comment-editor-toolbar";
import type { ModalType } from "./comment-insert-modal";
import { CuckooInsertModal } from "./comment-insert-modal";
...
export const CuckooCommentEditor = ({
  onSubmit,
  isSubmitting,
  autoFocus,
  onCancel,
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[80px] w-full bg-transparent py-2 text-sm focus:outline-none cuckoo-text-75 max-w-none",
      },
    },
  });
  ...
  return (
    <div className="relative rounded-(--cuckoo-radius) border border-(--cuckoo-input-border) bg-transparent transition-all duration-300 overflow-hidden focus-within:bg-(--cuckoo-primary)/5 focus-within:border-(--cuckoo-primary)/50 focus-within:shadow-sm">
      {/* Toolbar */}
      <div className="border-b border-black/5 dark:border-white/5 px-1 py-0.5">
        <CuckooCommentEditorToolbar
          editor={editor}
      ...
      <div className="flex items-center justify-between px-4 pb-3 pt-2 border-t border-black/5 dark:border-white/5">
        <span className="cuckoo-text-30 text-xs">
        ...
        <div className="flex items-center gap-3">
          {onCancel && (
            <button
              onClick={onCancel}
              className="cuckoo-text-50 text-sm hover:cuckoo-text-75 transition-colors"
            >
          ...
          <button
            ...
            className="cuckoo-btn-primary h-8 px-4 text-sm rounded-lg gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
          >
