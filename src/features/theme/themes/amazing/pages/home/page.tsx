...
/**
 * Enhanced Home Page — Amazing Theme
 *
 * Features:
 * - Alternating slide-in-left / slide-in-right entrance for cards
 * - Staggered delay for waterfall effect
 * - "View all" button with glow pulse
 * - IntersectionObserver-triggered entrance animations
 */
export function HomePage({ posts, pinnedPosts, popularPosts }: HomePageProps) {
  const delayOffset = 80;
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col rounded-(--fuwari-radius-large) bg-(--fuwari-card-bg) py-1 md:py-0 md:bg-transparent md:gap-4">
        {mergedPosts.map(({ post, pinned, popular }, i) => {
          ...
          const animationName =
            i % 2 === 0 ? "amazing-slide-in-left" : "amazing-slide-in-right";
          return (
            <div
              key={post.slug}
        ...
        >
          <Link
            to="/posts"
            className="fuwari-btn-regular mx-6 rounded-lg h-10 px-6 mt-4 flex items-center justify-center mb-4 md:mb-0 md:mx-auto hover:shadow-[0_0_24px_rgba(var(--amazing-primary-rgb),0.2)] transition-all duration-300 hover:scale-105 active:scale-95"
          >
